...
def fetch_market_prices(api_key, state, market, commodity, start_date, end_date, limit=999999999999):
    ...
    url += f"&filters[State.keyword]={state}&filters[Market.keyword]={market}&filters[Commodity.keyword]={commodity}"
...
def update_daily_prices(api_key, states, markets, commodities, daily_file="data/daily.csv"):
    """Fetch prices for all markets and commodities and update the daily.csv file."""
    #st.write("In Update daily prices")
    try:
        # Get today's date in 'YYYY-MM-DD' format
        today_date = datetime.today().strftime('%Y-%m-%d')
        file_name = f"{today_date}.csv"
        file_path = os.path.join("fetch", file_name)
        ...
        if os.path.exists(file_path):
            ...
            with open(file_path, 'w') as file:
                file.write('')  # Write an empty file or add headers as needed
            print(f"File '{file_name}' created in the folder.")
            ...
            for state in states:
                for market in markets:
                    for commodity in commodities:
                        try:
                            data = fetch_market_prices(api_key, state, market, commodity, today, today)
                            if not data.empty:
                                all_data = pd.concat([all_data, data], ignore_index=True)
                        except Exception as e:
                            print(f"Error checking/creating the file: {e}")
                        
            
            if not all_data.empty:
                # Save or append to daily.csv
                if os.path.exists(daily_file):
                    existing_data = pd.read_csv(daily_file)
                    all_data = pd.concat([existing_data, all_data], ignore_index=True)
                
                all_data.to_csv(daily_file, index=False)
                print(f"Data successfully updated in {daily_file}.")
            else:
                print("No data fetched for the current day.")
...
if not pre_existing_data.empty:
    ...
    ### run daily update in backend
    
    # thread = threading.Thread(target=update_daily_prices, args=("579b464db66ec23bdd000001d74c0d0c21a44b0572ae636312525017", states, markets, commodities))
...
update_daily_prices("579b464db66ec23bdd000001d74c0d0c21a44b0572ae636312525017", states, markets, commodities)
